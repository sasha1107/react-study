...
<WhiteCard title='You will learn'>
    - "배칭"이란 무엇이며 React가 그것을 사용하여 여러 상태 업데이트를 처리하는 방법
...
<WhiteCard title='요약'>
- Setting state는 기존 렌더링의 변수를 변경하지 않지만, 새로운 렌더링을 요청한다.
- 리액트는 이벤트 핸들러가 모든 실행을 마친 후 상태 업데이트를 처리한다. 이것은 batching(일괄 처리)이라고 불린다.
- 한 이벤트에서 일부 상태를 여러 번 업데이트하려면 업데이터 함수(updater function)을 사용할 수 있다.
    - ex) `setNumber(n => n + 1)`
</WhiteCard>