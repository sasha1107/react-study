...
다른 예시: 자동차 조수석에서 운전자에게 어디로 가야 하는지 알려준다고 상상해보자.
...
디자이너와 함께 일한다면, 다양한 "시각적 상태(visual states)"에 대한 목업을 보았을 것이다.

리액트는 디자인과 컴퓨터 사이언스의 교차점에 서 있기 때문에, 이 두 가지 아이디어는 모두 영감의 원천이다. 

<br/>

먼저, 사용자에게 표시될 수 있는 UI의 다양한 "상태들(states)"을 모두 시각화해야 한다. 

- Empty : Form 에 "Submit" 버튼이 비활성화되어 있다.
- Typing : Form에 "Submit" 버튼이 활성화되어 있다.
- Submitting : Form이 완전히 비화성화되었다. 스피너가 표시된다.
- Success: Form 대신 "Thank you" 메시지가 표시된다.
- Error: Typing 상태와 동일하지만, 추가 오류 메시지가 표시된다.
...
컴포넌트에 시각적 상태가 많은 경우, 한 페이지에 표시하는 것이 편리활 수 있다.
...
<br/>
이러한 페이지를 흔히 "living styleguides" 또는 "storybook"이라고 부른다.
</DeepDive>
- - -
...
- <b>Computer inputs</b> : 네트워크 응답 도착, timeout 완료, 이미지 로딩
...
<img src='https://react.dev/_next/image?url=%2Fimages%2Fdocs%2Fdiagrams%2Fresponding_to_input_flow.png&w=1920&q=75' alt='form states flow chart'/>
이 흐름을 시각화해보세요. 이러한 방식으로 많은 플로우를 스캐치하고 구현하기 훨씬 전에 버그를 분류할 수 있다.
...
예를 들어, `status`가 `success일 때, null이 아닌 `error`는 의미가 없다.
